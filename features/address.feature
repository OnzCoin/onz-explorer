Feature: Address page
  Scenario: should show title, summary, and transactions
    Given I'm on page "/address/15323650579610211509Z"
    ...
    And I should see table "summary" containing:
      ...
      | Public Key    | 55f7d4a2b4d76cd7beb3c65cdacc460d529d4f461c318b1e7daa4318c983894b |
      | Total balance | 99,675,450.108366 ONZ                                            |
      | Transactions  | 2 1329                                                           |
    ...
    And I should see table "transactions" with 50 rows starting with:
      ...
      |----------------|---------------------------|-----------------------|--------------------------|-----------|---------|---------------|
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 15323650579610211509Z | /standby_\d{3}\|\d{20}L/ | 1,000 ONZ | 0.1 ONZ | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 15323650579610211509Z | /standby_\d{3}\|\d{20}L/ | 1,000 ONZ | 0.1 ONZ | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 15323650579610211509Z | /standby_\d{3}\|\d{20}L/ | 1,000 ONZ | 0.1 ONZ | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 15323650579610211509Z | /standby_\d{3}\|\d{20}L/ | 1,000 ONZ | 0.1 ONZ | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 15323650579610211509Z | /standby_\d{3}\|\d{20}L/ | 1,000 ONZ | 0.1 ONZ | Confirmed     |
