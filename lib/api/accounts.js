...
module.exports = function (app) {
	...
	function Account() {
		this.validAddress = address =>
			(typeof address === 'string' && address.match(/^ONZ[a-zA-Z0-9]{33}$/g));
		...
		this.getAccount = (address, cb) =>
			request.get({
				url: `${app.get('onz address')}/api/accounts?address=${address}`,
				json: true,
			}, (err, response, body) => {
		...
		this.getAccountByPublicKey = (publicKey, cb) =>
			request.get({
				url: `${app.get('onz address')}/api/accounts?publicKey=${publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/delegates/get?publicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getVotes = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/accounts/delegates/?address=${account.address}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getVoters = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/delegates/voters?publicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getIncomingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/transactions?recipientId=${account.address}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getOutgoingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/transactions?senderId=${account.address}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getForged = (account, cb) => {
			...
			return request.get({
				url: `${app.get('onz address')}/api/delegates/forging/getForgedByAccount?generatorPublicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getTopAccounts = (query, error, success) => {
		...
		return request.get({
			url: `${app.get('onz address')}/api/accounts/top?&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
