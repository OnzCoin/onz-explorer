...
module.exports = function (app) {
	this.getBlockHeight = function (error, success) {
		request.get({
			url: `${app.get('onz address')}/api/blocks/getHeight`,
			json: true,
		}, (err, response, body) => {
	...
	this.getBlockByHeight = function (height, error, success) {
		request.get({
			url: `${app.get('onz address')}/api/blocks?height=${height}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
		...
		this.getBlockHeight(
			...
			(data) => {
				if (data.success === true) {
					return request.get({
						url: `${app.get('onz address')}/api/blocks?orderBy=height:desc&limit=20&offset=${blocks.offset(n)}`,
						json: true,
					}, (err, response, body) => {
						if (err || response.statusCode !== 200) {
							return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	function Block() {
		...
		this.getBlock = function (blockId, height, cb) {
			request.get({
				url: `${app.get('onz address')}/api/blocks/get?id=${blockId}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getBlockStatus = function (query, error, success) {
		request.get({
			url: `${app.get('onz address')}/api/blocks/getStatus`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
