...
module.exports = function (app) {
	...
	function Active() {
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('onz address')}/api/delegates/?orderBy=rate:asc&limit=101`,
				json: true,
			}, (err, response, body) => {
		...
		this.getForged = function (delegate, cb) {
			request.get({
				url: `${app.get('onz address')}/api/delegates/forging/getForgedByAccount?generatorPublicKey=${delegate.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			url: `${app.get('onz address')}/api/delegates/?orderBy=rate:asc&limit=${delegates.limit}&offset=${delegates.actualOffset}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
	...
	function Registrations() {
		this.getTransactions = function (cb) {
			request.get({
				url: `${app.get('onz address')}/api/transactions/?orderBy=timestamp:desc&limit=5&type=2`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = function (tx, cb) {
			request.get({
				url: `${app.get('onz address')}/api/delegates/get?publicKey=${tx.senderPublicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	function Votes() {
		this.getVotes = (cb) => {
			request.get({
				url: `${app.get('onz address')}/api/transactions/?orderBy=timestamp:desc&limit=5&type=3`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getNextForgers = function (query, error, success) {
		request.get({
			url: `${app.get('onz address')}/api/delegates/getNextForgers?limit=101`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	this.getDelegateProposals = function (error, success) {
		...
		let nextPage = false;
		const limit = 25;
		const url = 'https://forum.onzcoin.com/viewforum.php?f=48&start=';
		/**
		 * @todo what?
		 */
	...
	function LastBlock() {
		this.getBlock = function (cb) {
			request.get({
				url: `${app.get('onz address')}/api/blocks?&orderBy=height:desc&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = function (block, cb) {
			request.get({
				url: `${app.get('onz address')}/api/delegates/get?publicKey=${block.generatorPublicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getLastBlocks = function (params, error, success) {
		...
		return request.get({
			url: `${app.get('onz address')}/api/blocks?orderBy=height:desc&generatorPublicKey=${params.publicKey}&limit=${params.limit}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: err });
	...
	this.getSearch = function (params, error, success) {
		if (!params || !params.match(/^ONZ[a-zA-Z0-9]{33}$)[0-9a-z.]+/i)) {
			return error({ success: false, error: 'Missing/Invalid username parameter' });
		}
		return request.get({
			url: `${app.get('onz address')}/api/delegates/search?q=${params}&limit=1`,
			json: true,
		}, (err, response, body) => {
