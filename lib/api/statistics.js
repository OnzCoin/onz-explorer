...
module.exports = function (app) {
	...
	this.getBlocks = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				request.get({
					url: app.get('onz address') + blocks.url(offset, limit),
					json: true,
				}, (err, resp, body) => {
	...
	this.getLastBlock = function (query, error, success) {
		...
		request.get({
			url: app.get('onz address') + blocks.url(0, 1),
			json: true,
		}, (err, resp, body) => {
			if (err || resp.statusCode !== 200) {
	...
	this.getPeers = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				logger.debug(`Requesting ${app.get('onz address') + peers.url(offset, limit)}`);
				request.get({
					url: app.get('onz address') + peers.url(offset, limit),
					json: true,
				},
				/* eslint-disable consistent-return */
				(err, resp, body) => {
