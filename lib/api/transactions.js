...
module.exports = function (app) {
	...
	const getSenderDelegate = function (transaction, cb, tmpDelegateCache) {
		...
		return request.get({
			url: `${app.get('onz address')}/api/delegates/get?publicKey=${transaction.senderPublicKey}`,
			json: true,
		}, (err, response, body) => {
	...
	const getRecipientPublicKey = function (transaction, cb, tmpDelegateCache) {
		...
		return request.get({
			url: `${app.get('onz address')}/api/accounts/getPublicKey?address=${transaction.recipientId}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				transaction.recipientPublicKey = null;
	...
	const getRecipientDelegate = function (transaction, cb, tmpDelegateCache) {
		...
		return request.get({
			url: `${app.get('onz address')}/api/delegates/get?publicKey=${transaction.recipientPublicKey}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				transaction.recipientDelegate = null;
	...
	const Transaction = function () {
		this.getDelegate = function (forger, cb) {
			request.get({
				url: `${app.get('onz address')}/api/delegates/get?publicKey=${forger.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
	...
	this.getTransaction = function (transactionId, error, success, url) {
		...
		return async.waterfall([
			function (waterCb) {
				request.get({
					url: app.get('onz address') + url + transactionId,
					json: true,
				}, (err, response, body) => {
					if (err || response.statusCode !== 200) {
	...
	this.getUnconfirmedTransactions = function (transactions, error, success) {
		async.waterfall([
			cb => request.get({
				url: `${app.get('onz address')}/api/transactions/unconfirmed`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	this.getLastTransactions = function (query, error, success) {
		request.get({
			url: `${app.get('onz address')}/api/transactions?orderBy=timestamp:desc&limit=20`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
		...
		return async.waterfall([
			function (cb) {
				...
				data.forEach((directionQuery, index) => {
					request.get({
						url: `${app.get('onz address')}/api/transactions?${directionQuery}`,
						json: true,
					}, (err, response, body) => {
						if (err || response.statusCode !== 200) {
	...
	this.getTransactionsByBlock = function (query, error, success) {
		...
		return async.waterfall([
			function (cb) {
				request.get({
					url: `${app.get('onz address')}/api/transactions?blockId=${query.blockId}&orderBy=timestamp:desc&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
					json: true,
				}, (err, response, body) => {
					if (err || response.statusCode !== 200) {
						return error({ success: false, error: (err || 'Response was unsuccessful') });
