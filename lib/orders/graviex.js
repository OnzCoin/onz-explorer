...
function GraviexOrders(...rest) {
	...
	this.url = 'https://graviex.net/api/v2/order_book?market=onzbtc&asks_limit=50&bids_limit=50';
	this.limit = 100;
	...
	this.acceptOrders = function (data) {
		...
		const buyKeys = Object.keys(data.result.bids);
		for (let i = 0; i < buyKeys.length; i++) {
			bids.push([data.result.bids[buyKeys[i]].price, data.result.bids[buyKeys[i]].volume]);
		}

		const sellKeys = Object.keys(data.result.asks);
		for (let i = 0; i < sellKeys.length; i++) {
			asks.push([data.result.asks[sellKeys[i]].price, data.result.asks[sellKeys[i]].volume]);
		}
...
util.inherits(GraviexOrders, AbstractOrders);
module.exports = GraviexOrders;
